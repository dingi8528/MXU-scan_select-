...
桌面端还会自动将“运行日志”保存为 `debug/ui-YYYY-MM-DD.log`（UTF-8），每个文本行都包含与面板一致的时间。文件名以程序启动日期为准，同一天再次启动会继续追加；若程序中途跨天，本次会话仍写入原文件。所有标签页共用该文件。富文本保留文字和图片替代文本，图片本身不写入文本文件。这些文件会随“导出日志”一起打包，也会被“清空日志”清理。界面仅显示最近的部分日志，文件保留当天已写入的记录。
