...
桌面端还会自动将“运行日志”保存为 `debug/ui-日期-会话标识-tab序号-分卷号.log`（UTF-8），内容为与面板一致的时间和消息文本。每次打开界面创建新会话，各标签页独立保存，单卷约 5 MiB 后续写新卷；单条日志保持完整。富文本保留文字和图片替代文本，图片本身不写入文本文件。这些文件会随“导出日志”一起打包，也会被“清空日志”清理。界面仅显示最近的部分日志，文件保留本次会话中已写入的记录。
